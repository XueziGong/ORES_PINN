...
class KLEField(nn.Module):
    ...
    def forward(self, z):
        ...
        # Return eigenfunctions (phi_z) alongside fluctuation for DNN orthogonal projection
        return fluctuation, phi_z

# ==========================================
# 4. Main Hybrid PINN Model (with ORP)
# ==========================================
...
class PINN(nn.Module):
    def __init__(self, device):
        ...
        # --- 5. Independent Learnable Leaky ORP Factors (gamma) ---
        # Initialize gamma to approx 0.1 (sigmoid(-2.1972) ≈ 0.1)
    ...
    def _orthogonalize(self, dnn_raw, phi_z, specific_gamma_logits):
        """
        Compute the orthogonal complement of the DNN output in the KLE space
        (with an independent soft leaky mechanism).
        """
        # 1. Numerator: Discrete inner product of DNN raw predictions and basis functions
        dot_D_phi = torch.sum(dnn_raw * phi_z, dim=0, keepdim=True)  # Shape: (1, nkl)
        
        # 2. Denominator: Discrete inner product of basis functions with themselves
        dot_phi_phi = torch.sum(phi_z * phi_z, dim=0, keepdim=True)  # Shape: (1, nkl)
        
        # 3. Projection coefficients (added 1e-8 to prevent division by zero)
        c = dot_D_phi / (dot_phi_phi + 1e-8)                         # Shape: (1, nkl)
        
        # 4. Compute the overlapping field (projection)
        projection = torch.sum(c * phi_z, dim=1, keepdim=True)       # Shape: (N, 1)
        
        # 5. Independent soft orthogonal stripping (leaky projection)
        gamma = torch.sigmoid(specific_gamma_logits)
    ...
    def forward(self, t, z):
        ...
        # --- B. Predict Parameters (Hybrid Reconstruction with ORP) ---
        # Note: Input to KLE is the physical z, input to DNN is normalized z
        
        # Ks
        fluct_kle_Ks, phi_z_Ks = self.kle_ln_Ks(z)
        dnn_raw_Ks = self.net_ln_Ks(z_norm)
        fluct_dnn_Ks = self._orthogonalize(dnn_raw_Ks, phi_z_Ks, self.gamma_logits_Ks)
        ln_Ks = self.mu_lnKs + fluct_kle_Ks + fluct_dnn_Ks

        # Alpha
        fluct_kle_alpha, phi_z_alpha = self.kle_ln_alpha(z)
        dnn_raw_alpha = self.net_ln_alpha(z_norm)
        fluct_dnn_alpha = self._orthogonalize(dnn_raw_alpha, phi_z_alpha, self.gamma_logits_alpha)
        ln_alpha = self.mu_lnalpha + fluct_kle_alpha + fluct_dnn_alpha

        # n
        fluct_kle_n, phi_z_n = self.kle_ln_n(z)
        dnn_raw_n = self.net_ln_n(z_norm)
        fluct_dnn_n = self._orthogonalize(dnn_raw_n, phi_z_n, self.gamma_logits_n)
