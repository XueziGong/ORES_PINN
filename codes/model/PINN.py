...
class StandardMLP(nn.Module):
    ...
    Structure: [input_dim, 64, 64, 64, 64, output_dim]
    """
...
class PINN(nn.Module):
    def __init__(self, device):
        ...
        # Parameter Network (Predicts Heterogeneity)
        # Inputs: z (1D) | Hidden: 64 | Output: 3D (ln_alpha, ln_n, ln_Ks)
        self.param_net = StandardMLP(input_dim=1, output_dim=3)
        
        # Fixed VGM Parameters (Assumed constant for this soil type)
        self.theta_r = 0.078
        self.theta_s = 0.43
    ...
    def forward(self, t, z):
        ...
        # 3. Predict Parameters (Alpha, n, Ks)
        # Output of param_net represents the fluctuation from the mean
        param_out = self.param_net(param_input)  # (N, 3)
        
        d_ln_alpha = param_out[:, 0:1]
        d_ln_n = param_out[:, 1:2]
        d_ln_Ks = param_out[:, 2:3]
